...
QString Labonatip_protocolWriter::createHeader()
{
	...
	header.append(tr("%% +---------------------------------------------------------------------------+\n"));
	header.append(tr("%% \n"));
	header.append(tr("%% Protocol Header V. 0.7 \n"));
	...
	header.append(tr("%%  -       3:  Wait (s)                      :  Wait in seconds \n"));
	header.append(tr("%%  -       4:  All Off                       :  All the valves 1 - 4 will be closed \n"));
	...
	header.append(tr("%%  -       9:  Pressure ON (mbar)            :  Value in mbar\n"));
	header.append(tr("%%  -       10: Pressure OFF (mbar)           :  Value in mbar\n"));
	header.append(tr("%%  -       11: Vacuum Recirculation (mbar)   :  Value in mbar \n"));
	header.append(tr("%%  -       12: Vacuum Switch (mbar)          :  Value in mbar \n"));
	header.append(tr("%%  -       13: Ask (string)                  :  Ask a message at the end of some operation \n"));
	header.append(tr("%%  -       14: Pumps Off                     :  Set the all the pumps to zero (pressure and vacuum) \n"));
	header.append(tr("%%  -       15: Wait sync                     :  Wait a sync signal \n"));
	header.append(tr("%%  -       16: Sync out                      :  Sync with external trigger \n"));
	header.append(tr("%%  -       17: Loop (num)                    :  All the commands inside the loop will run cyclically \n"));
	header.append(tr("%%  - \n"));
