...
QString protocolWriter::createHeader()
{
	...
	header.append(tr("%% +---------------------------------------------------------------------------+\n"));
	header.append(tr("%% \n"));
	header.append(tr("%% Protocol Header V. 0.8 \n"));
	...
	header.append(tr("%%  -       12: Pressure ON (mbar)            :  Value in mbar\n"));
	header.append(tr("%%  -       13: Pressure OFF (mbar)           :  Value in mbar\n"));
	header.append(tr("%%  -       14: Vacuum Recirculation (mbar)   :  Value in mbar \n"));
	header.append(tr("%%  -       15: Vacuum Switch (mbar)          :  Value in mbar \n"));
	header.append(tr("%%  -       16: Ask (string)                  :  Ask a message at the end of some operation \n"));
	header.append(tr("%%  -       17: Pumps Off                     :  Set the all the pumps to zero (pressure and vacuum) \n"));
	header.append(tr("%%  -       18: Wait sync                     :  Wait a sync signal \n"));
	header.append(tr("%%  -       19: Sync out                      :  Sync with external trigger \n"));
	header.append(tr("%%  -       20: Loop (num)                    :  All the commands inside the loop will run cyclically \n"));
	header.append(tr("%%  - \n"));
