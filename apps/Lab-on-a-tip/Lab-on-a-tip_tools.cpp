...
void Labonatip_tools::addMacroCommand()
{
	...
	if (ui_tools->treeWidget_macroTable->topLevelItemCount() < 1 ||
		...
		ui_tools->treeWidget_macroTable->resizeColumnToContents(0);
...
bool Labonatip_tools::checkValidity(QTreeWidgetItem *_item, int _column)
{
	...
	case 8:{
		...
		qobject_cast<QComboBox*>(ui_tools->treeWidget_macroTable->itemWidget(_item, 0))->setCurrentIndex(0);
		QMessageBox::warning(this, "Warning",
...
void Labonatip_tools::commandChanged(int _idx)
{
	...
	checkValidity(ui_tools->treeWidget_macroTable->currentItem(), _idx);
}
...
void Labonatip_tools::createNewCommand(QTreeWidgetItem & _command, macroCombobox & _combo_box)
{
